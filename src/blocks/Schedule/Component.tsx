        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {schedules.map((item) => {
              ...
              return (
                <tr className="border-t even:bg-slate-50" key={item.id}>
                  <td className="p-4 font-semibold">{item.dayOfWeek}</td>
                  ...
                  <td className="p-4 capitalize">{item.mode}</td>
        ...
        {schedules.map((item) => {
          ...
          return (
            <article className="rounded-2xl border bg-white p-5 shadow-sm" key={item.id}>
              <p className="text-sm font-bold uppercase tracking-wide text-blue-700">{item.dayOfWeek}</p>
              <h3 className="mt-2 text-xl font-bold text-navy">
                {course ? <LocalizedText english={course.titleEn} sinhala={course.titleSi} /> : item.batchLabel}
              </h3>
              <p className="mt-2">{item.startTime} – {item.endTime}</p>
              <p className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
                <MapPin className="size-4" />{item.location} · {item.mode}
              </p>
            </article>
          )
        })}
