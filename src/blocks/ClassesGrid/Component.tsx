        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {classes.map((course) => (
            <article className="group rounded-3xl border bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-lg" key={course.id}>
              ...
              <h3 className="mt-5 text-2xl font-bold text-navy">{localized(locale, course.titleEn, course.titleSi)}</h3>
              <p className="mt-3 min-h-20 leading-7 text-muted-foreground">{localized(locale, course.shortDescriptionEn, course.shortDescriptionSi)}</p>
