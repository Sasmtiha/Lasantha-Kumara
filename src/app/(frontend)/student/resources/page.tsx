...
export default async function StudentResourcesPage() {
  ...
  return <div className="container py-12"><p className="section-kicker">Library</p><h1 className="section-title">Learning resources</h1><div className="mt-8 grid gap-4 md:grid-cols-2">{resources.map((item) => { const file = typeof item.file === 'object' ? item.file : null; const href = item.externalUrl || file?.url; return <article className="rounded-2xl border bg-white p-6" key={item.id}><p className="text-sm font-bold uppercase text-blue-700">{item.resourceType}</p><h2 className="mt-2 text-xl font-bold text-navy">{item.title}</h2><p className="mt-2 text-muted-foreground">{item.description}</p>{href ? <a className="mt-5 inline-block font-bold text-blue-800" href={href} rel="noreferrer" target="_blank">Open resource →</a> : null}</article> })}{!resources.length ? <p className="text-muted-foreground">Resources will appear here for your approved classes.</p> : null}</div></div>
}
