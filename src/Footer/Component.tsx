...
export async function Footer() {
  ...
  return (
    <footer className="site-footer relative mt-auto overflow-hidden bg-[#090a0e] text-white">
      <span aria-hidden className="absolute -bottom-16 left-0 text-[16vw] font-semibold leading-none text-white/[.025]">IEM</span>
      <div className="premium-container relative grid gap-12 py-20 md:grid-cols-2 lg:grid-cols-4">
        <div className="md:col-span-2 lg:col-span-1">
          <Link className="flex items-center" href="/"><Logo variant="dark" /></Link>
        ...
        <div>
          ...
          <div className="mt-5 flex gap-3">
            {settings.facebookUrl ? <a aria-label="IEM on Facebook" className="grid size-10 place-items-center rounded-md border border-white/15 text-white/70 hover:border-white hover:text-white" href={settings.facebookUrl} rel="noreferrer" target="_blank"><Facebook className="size-4" /></a> : null}
            {settings.youtubeUrl ? <a aria-label="IEM on YouTube" className="grid size-10 place-items-center rounded-md border border-white/15 text-white/70 hover:border-white hover:text-white" href={settings.youtubeUrl} rel="noreferrer" target="_blank"><Youtube className="size-4" /></a> : null}
          </div>
        </div>
      </div>
      <div className="premium-container relative border-t border-white/10 py-6 text-sm text-white/40">
        {footerData.copyrightText || `© ${new Date().getFullYear()} IEM. All rights reserved.`}
      </div>
      {settings.whatsappNumber ? (
        <a
          aria-label="Contact IEM on WhatsApp"
